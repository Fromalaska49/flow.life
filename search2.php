...
if(!isset($_GET['q'])||$_GET['q']==''){
	...
	$query = '%' . $q . '%';
	if($_GET['church']==1){
		$sql = "SELECT * FROM rank JOIN pages ON rank.pageid=pages.id WHERE name LIKE '$query' ORDER BY rank DESC";
		...
		if ($num > 0){
			...
			while($row = mysql_fetch_array($result)) {
				...
				echo ('<li style="list-style-type:none;text-align:left;"><a style="" href="page.php?id=' . $pageid . '"><img src="'.$ppimgurl.'" style="height:80px;width:80px;margin-right:10px;" /><p>' . $name . '</p><p>'.$type.'</p></a></li>');
			}
			echo ('</ul>');
		}
		else {
		...
		$sql = "SELECT * FROM rank WHERE church='0' AND name LIKE '$query' ORDER BY rank DESC";
		$result = mysql_query($sql);
		$num = mysql_num_rows($result);
		if ($num > 0){
			while($row = mysql_fetch_array($result)) {
				...
				$pageid = $row['pageid'];
				echo ('<ul>');
				echo ('<li style="list-style-type:none;text-align:left;"><a style="" href="page.php?id=' . $pageid . '">' . $name . '</a></li>');
				echo ('</ul>');
			}
		}
		else {
...
Listen to my voice, for I am Adonai.
