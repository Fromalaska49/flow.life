	...
	<div style="overflow:auto;">
		...
		<div>
			...
			<div style="float:right;">
				<?php echo('<img src="images/upload/users/s/' . $ppimgurl . '" style="height:80px;" />'); ?>
			</div>
		</div>
	</div>
	<hr />
		...
		<div>
			<div style="float:left;">
				<form>
					...
					<br />
					<br />
					<input type="submit" value="Add Email" class="button" onmousedown="this.className='buttonactive'" onmouseout="this.className='button'" onmouseup="this.className='button'" />
				</form>
			</div>
			<div style="float:right;">
				<?php $result=mysql_query("SELECT * FROM `altemails` WHERE `uid`='$uid'");while($emails=mysql_fetch_array($result)){echo(htmlentities($emails['altemail']).'<br />');} ?>
