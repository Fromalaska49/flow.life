				...
				<div id="feedbody">
					...
					while($myrow = mysql_fetch_array($result2a)){
						...
						if(($myrow["media"]!==0) && ($myrow["media"]!=='') && ($myrow["media"]!=='0')){
							$media = '<a href="http://flow.life/images/upload/pages/l/'.$myrow["media"].'"><img style="max-width:300px;" src="http://flow.life/images/upload/pages/m/'.$myrow["media"].'" /></a>';
						}
						else{
							$media='';
						}
...
//echo ('-->');
//$weekday = date('w', $time);
						$usql = "SELECT * FROM `pages` WHERE `id`='$profiledpage'";
						$uresult = mysql_query($usql);
						$uinfo = mysql_fetch_array($uresult);
						$name = htmlentities($uinfo["name"]);
						$pimgurl = $uinfo["ppimgurl"];
...
$num = "";
						...
						echo ('<div class="container"><div class="containerhead" style=""><a href="page.php?id=' . $pageid . '"><img style="height:40px;float:left;" src="http://flow.life/images/upload/pages/p/' . $pimgurl . '" /></a><h4 class="ttl" style=""><a href="page.php?id=' . $pageid . '"><div class="pcheader">' . $name . '</div></a><div class="pcheader" style="float:right;">' . '</div></h4></div><div class="ptextcontainer">' . $annotation . $post . $media . '</div><div class="containerfoot"><div style="display:inline;padding:5px;float:left;color:#AAAAAA;font-size:15px;padding:0px;margin:2px;">' . $time . '</div><div style="display:inline;padding:5px;color:#AAAAAA;font-size:20px;padding:0px;margin:2px;" id="like' . $pid . '">' . $printnumber . '</div><!--<div style="display:inline;padding:5px;" onclick="like(' . $pid . ')" title="Thumbs-Up"><img class="thumbsup" src="thumbs-up-' . $thumbcolor . '.png" id="' . $pid . '" />' . '</div><div style="display:inline;padding:5px;"><a href="pagecomments.php?id=' . $pid . '" title="Comment"><img style="width:20px;height:20px;" src="comment.png" /></a></div>--></div>');
...
//$cweekday = $ct['weekday'];
					...
					if($postquantity>30){
						...
						echo("<p style=\"color:#888888;\">That's it!</p><br />");
					}
					else{
...
user-select: none;">
		...
		<?php
		if($pageid==$profiledpage){
			$dfvalue = "<div id=\"dfbutton\" class=\"button\" style=\"position:absolute;top:210px;left:0px;\"  onmousedown=\"this.className='buttonactive'\" onmouseup=\"this.className='button'\" onmouseout=\"this.className='button'\" onclick=\"this.className='button';window.location='settings.php';\">Edit</div>";
		}
		else{
			$dfvalue='';
		}
		...
		echo ("<a href=\"http://flow.life/images/upload/pages/l/".$pimgurl."\"><img src=\"http://flow.life/images/upload/pages/m/".$pimgurl."\" style=\"width:250px;\" /></a><br /><center><h3>".$name."</h3></center>".$dfvalue."<p class=\"profileduserdata\"><a href=\"mailto:".$email."\" style=\"color:white;\" onmouseover=\"this.style.textDecoration='underline'\" onmouseout=\"this.style.textDecoration='none'\">" . $email . "</a></p><p class=\"profileduserdata\">" . $tel . "</p>".$address);
		?>
		</div>
	</body>
</html>
