...
$result = mysql_query("SELECT * FROM passwordreset WHERE key='$sanitizedkey'");
$n=mysql_num_rows($result);
if($n==1){
	$row=mysql_fetch_array($result);
	if((time()-$row['time'])>86400){
		if($_POST['password1']==$_POST['password2']){
			$cryptedpassword=mysql_real_escape_string(md5($_POST['password1']));
			$email=mysql_real_escape_string($row['email']);
			mysql_query("UPDATE users SET password='$password' WHERE email='$email'");
			mysql_query("DELETE * FROM passwordreset WHERE key='$sanitizedkey'");
		}
		else{
			//hackers again
		}
	...
	//invalid key
	//probably hacker or web crawler
}
else{
	mysql_query("DELETE * FROM `passwordreset` WHERE `key`='$sanitizedkey'");
}
...
if ($num > 0) {
	...
	header('Location: loginretry.php?email='.$email);
}
?>