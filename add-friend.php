...
<html>
	<head>
		...
		<?php require "friend-request-response-javascript.php"; ?>l>
		<style type="text/css">
		td
		{
		padding:10px;
...
<form action="script-add-friend.php" method="post">
<br />
	<div>
		<input type="text" value="" style="min-width:400px;width:600px;border-radius:5px 0px 0px 5px;margin-right:-3px;" onclick="this.value='';" class="searchinput" name="email" placeholder="Email" />
		<input type="submit" value="Add" style="border-radius:0px 5px 5px 0px;margin-left:-3px;position:relative;top:-1px;" class="button" onmousedown="this.className='buttonactive'" onmouseout="this.className='button'" onmouseup="this.className='button'" />
		<p style="width:643px;text-align:left;">
		Enter your friend's email here. If they have an account, they will recieve a friend request, otherwise they will recieve your posts via email until they create an account.
		</p>
	</div>
</form>
<?php
if(!isset($_GET['q'])||$_GET['q']==''){
	...
	if ($num > 0){
		...
		echo('<li style="list-style-type:none;color:;"><b>No Results</b> for ' . $q . '</li>');
	}
}
?>
</div>
